...
function buildStatusResponse(
  ...
  // ✅ Status URL now points to GET /api/mining/click?key=...
...
export async function POST(request: NextRequest) {
  ...
  const processDirect = async (successOutcome: string) => {
    try {
      ...
      return respond(NextResponse.json({ error: "Unable to process mining request" }, { status: 500 }), {
...
export async function GET(request: NextRequest) {
  ...
  // Mirror the same response codes/headers as POST
  let statusCode = 202
  const headers: Record<string, string> = { "Cache-Control": "no-store" }
  ...
  return respond(NextResponse.json({ status }, { status: statusCode, headers }), { outcome: status.status })
}
