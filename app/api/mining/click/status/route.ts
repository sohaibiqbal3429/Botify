...
export async function GET(request: NextRequest) {
  ...
  if (!user) {
    return respond(NextResponse.json({ error: "Unauthorized" }, { status: 401, headers: { "Cache-Control": NO_STORE } }), {
  ...
  if (!key) {
    return respond(NextResponse.json({ error: "Missing idempotency key" }, { status: 400, headers: { "Cache-Control": NO_STORE } }), {
  ...
  if (!status || status.userId !== user.userId) {
    return respond(NextResponse.json({ error: "Status not found" }, { status: 404, headers: { "Cache-Control": NO_STORE } }), {
      outcome: "not_found",
    })
  }
