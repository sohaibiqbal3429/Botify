    ...
    <div className="flex-1 min-w-0 w-full space-y-6">
      <header className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Transfer USDT to the platform wallets and track your credited balance.</p>
        </div>
      ...
      <section>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Percentages are of the <strong>deposit amount</strong>. Your status is recalculated <strong>after each deposit</strong>;
              bonuses for that deposit use your <strong>post-update</strong> status.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <p className="text-xl font-semibold text-primary">
                {isActive
                  ? `${pct(DEPOSIT_SELF_PERCENT_ACTIVE)} of every deposit`
                  : `Become Active to unlock ${pct(DEPOSIT_SELF_PERCENT_ACTIVE)}`}
              </p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">L1 referral share</p>
              <p className="text-xl font-semibold">{pct(DEPOSIT_L1_PERCENT)} of each deposit (always, if you have an L1)}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">L2 referral share</p>
              <p className="text-xl font-semibold">
                {isActive ? `${pct(DEPOSIT_L2_PERCENT_ACTIVE)} when you are Active` : "0% until you are Active"}
              </p>
            </div>
          </CardContent>
        </Card>
