...
const LEVEL_TASK_DEFINITIONS: Record<number, LevelTaskDefinition[]> = {
  1: [
    ...
    },
    {
      ...
      completed: (context) => context.depositTotal >= LEVEL1_DEPOSIT_TARGET,
    },
  ],
  2: [
    {
    ...
    },
    {
      ...
      description: "Maintain an active balance above 50 USDT.",
      reward: 0.75,
      type: "balance",
      target: LEVEL2_BALANCE_THRESHOLD,
      progress: (context) => Math.min(context.balanceCurrent, LEVEL2_BALANCE_THRESHOLD),
      completed: (context) => context.balanceCurrent >= LEVEL2_BALANCE_THRESHOLD,
    },
    {
      id: "level2-deposit-milestone",
      title: "Reach 100 USDT Deposits",
      description: "Increase your total deposits to at least 100 USDT.",
      reward: 0.6,
      type: "deposit",
      target: FIRST_DEPOSIT_TARGET,
      progress: (context) => Math.min(context.depositTotal, FIRST_DEPOSIT_TARGET),
      completed: (context) => context.depositTotal >= FIRST_DEPOSIT_TARGET,
    },
  ],
  3: [
    {
...
function buildLegacyTasks(
  ...
  return [
    ...
    },
    {
      ...
      reward: 0.5,
      type: "profile",
      progress: Math.min(context.profileStepsCompleted, PROFILE_COMPLETION_TARGET),
      target: PROFILE_COMPLETION_TARGET,
      completed: context.profileStepsCompleted >= PROFILE_COMPLETION_TARGET,
      rewardClaimed: claimedTaskIds.has("complete-profile"),
    },
  ]
}
...
export async function getTasksForUser(userId: string): Promise<UserTask[]> {
  ...
  const [balanceDoc, claimedRewards, directReferrals] = await Promise.all([
