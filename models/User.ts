...
UserSchema.pre("findOneAndUpdate", async function (this: mongoose.Query<IUser | null, IUser>) {
  const model = this.model as mongoose.Model<IUser>
  ...
  const existing = await model.findOne(filter as any).select({ _id: 1 }).lean<{ _id: mongoose.Types.ObjectId | string }>()
